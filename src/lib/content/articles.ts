/**
 * Higher-level article queries (organizations, qualifications, related-content
 * scoring, link graph) built on top of the low-level content-data/jobs/*.json
 * loader in @/content/articles.
 *
 * RULE 62: Content source and frontend rendering must be separate.
 * Frontend never scrapes — the discovery pipeline (scripts/discover-and-generate.mjs)
 * writes normalized JSON files; this layer only ever reads them.
 */
